...
function Loop() {
    if (window.Playing && Api.ParticleContainer.children.length > 0) {
        Array.from(Api.ParticleContainer.children).forEach(Particle => {
            ...
            const ParticleColorArray = Api.Numeric.RgbToArray(PartAttributes.Color);

            Particle.style.backgroundColor = !Particle.hasAttribute("nocolor") ? `rgb(${parseInt(ParticleColorArray[0]) + (Temp / Math.pow(2, 8))}, ${parseInt(ParticleColorArray[1])}, ${parseInt(ParticleColorArray[2])})` : PartAttributes.Color;
                ...
                if (OtherParticle !== Particle) {
                    ...
                    if (!(ParticleRight < OtherParticleLeft || 
                        ...
                        if (Api.Particle.IsPlaceOccupied(NewPositionX, NewPositionY)) {
                            ...
                            const OtherColor = Api.Numeric.RgbToArray(OtherParticle.getAttribute("Color"));
                                ...
                                if (ReactData[0]) {
                                    ...
                                    ReactiveElements.forEach(ReactiveName => {
                                        if (OtherName.includes(ReactiveName)) {
                                            ...
                                            const RandomReactionTemp = Math.floor(Api.Numeric.Random(AverageTemp, AverageMeltTemp));
                                            ...
                                            if (ReactionAmalgamateElements && ReactionAmalgamateElements[0]) {
                                                setTimeout(() => {
                                                    const AmalgamationPower = Math.floor(Api.Numeric.Random(-window.Effects.Powder.Power, window.Effects.Powder.Power));
                                            ...
                                            const RandomVaporizeChance = Math.floor(Api.Numeric.Random(0, 100));
                                            if (RandomVaporizeChance >= 75 + Reactivity) {
                                                const RandomParticleChance = Math.floor(Api.Numeric.Random(0, 100));
                                                if (RandomParticleChance >= 75 + Reactivity) Particle.remove();
                                                else OtherParticle.remove();
                                            } else {
                                                if (ReactionSideProducts.length > 0) {
                                                    const SideProductChance = Math.floor(Api.Numeric.Random(0, 100));
            ...
            if (Type.includes("Radioactive") && !Type.includes("Light")) {
                if (Math.random() < Math.pow(10, -2)) {
                    ...
                    Particle.setAttribute("Temp", Temp + Api.Numeric.Random(22, 128));
                    if (Math.random() < Math.pow(10, -2)) {
