...
function Loop() {
    if (Api.ParticleContainer.children.length > 0) {
        Array.from(Api.ParticleContainer.children).forEach(Particle => {
            ...
            const ColorArray = getComputedStyle(Particle).backgroundColor
                .match(/\d+/g)
                .map(Number);
                    
            const AlphaTemp = Math.min(Temp || 0, parseInt(Particle.dataset.Melt) || 0);

            const NewColor = window.Effects.Powder.Enabled
                ? `rgb(${ColorArray[0] + AlphaTemp}, ${ColorArray[1]}, ${ColorArray[2]})`
                : Particle.Color;

            Particle.style.backgroundColor = `${NewColor} !important`;
