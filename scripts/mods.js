...
function ArrayCompare(Arr1, Arr2) {
    ...
    for (let Index = 0; Index < Arr1.length; Index++) {
        ...
        if (typeof Element1 === "object" && typeof Element2 === "object") {
            if (!ObjectCompare(Element1, Element2)) {
                return false;
            }
...
if (!ArrayCompare(LastMods, Mods)) {
    ...
    if (LastMods.length > Mods.length) ContentMessage = `${Math.abs(LastMods.length - Mods.length)} mods removed.`;  
    else ContentMessage = `${Math.abs(LastMods.length - Mods.length)} new mods added.`;
