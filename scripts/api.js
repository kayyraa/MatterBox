    ...
    static GeneratePart(Pos = {
        ...
        if (Particle && !this.IsPlaceOccupied(X, Y) || IgnoreOccupation) {
            ...
            PartStates.forEach(State => {
                if (Particle[State]) ParticleElement.setAttribute(State, Particle[State]);
                else ParticleElement.setAttribute(State, "NULL");
            });

            Additional.forEach(Property => {
                ParticleElement.setAttribute(Property.Name, Property.Value);
                ParticleElement.dataset[`nc${Property.Name}`] = Property.Value;
            });
