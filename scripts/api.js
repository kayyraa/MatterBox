...
export class Particle {
    ...
    static BurnParticle(Particle) {
        ...
        const Loop = () => {
            ...
            if (FlammableData[2] <= 0) {
                ...
                Particle.setAttribute("Flammable", JSON.stringify([true, FlammableData[1], FlammableData[2] - parseInt(Api.Number.Random(8, 32))]));
            }
    ...
    static GeneratePart(Pos = {
        ...
        if (Particle && !this.IsPlaceOccupied(X, Y) || IgnoreOccupation) {
            const ColorArray = Api.Number.RgbToArray(Particle.Color);
            ...
            const Result = `rgb(${ColorArray[0] + Particle.Melt}, ${ColorArray[1] + (Particle.Melt / 8)}, ${ColorArray[2] - (Particle.Melt / 64)})`;
...
export class Number {
    ...
    static Matrix = class {
        static Add(Matrix0, Matrix1) {
            const Result = new Number.Matrix.Init(Matrix0.Rows, Matrix0.Cols);
        ...
        static Multiply(Matrix0, Matrix1) {
            const Result = new Number.Matrix.Init(Matrix0.Rows, Matrix1.Cols);
            for (let i = 0; i < Matrix0.Rows; i++) {
...
export class Storage {
    ...
    Append() {
        ...
        if (!this.Storage.StorageId) {
            this.Storage.StorageId = `${Characters[Math.floor(Math.random() * Characters.length)].toUpperCase()}${(Math.random().toString(36).substring(2, 8))}`;
        }

        const StorageArray = this.Storage.StorageItems.map((Item, Index) =>
            this.Storage.Options.IndexItems ? [Index, Item] : Item
        );

        localStorage.setItem(this.Storage.StorageId, JSON.stringify(StorageArray));
        localStorage.setItem(`Backup:${this.Storage.BackupId || this.Storage.StorageId}`, JSON.stringify(StorageArray));
    }
    ...
    AddItem(Item) {
        ...
        if (!this.Storage.StorageSize || StorageArray.length <= this.Storage.StorageSize) {
            localStorage.setItem(this.Storage.StorageId, JSON.stringify(StorageArray));
        }
    }
    ...
    UpdateItem(OldItem, NewItem) {
        ...
        const StorageArray = JSON.parse(localStorage.getItem(this.Storage.StorageId));
        const UpdatedStorageArray = StorageArray.map(StoredItem =>
    ...
    GetItems() {
        return JSON.parse(localStorage.getItem(this.Storage.StorageId) || "[]");
    }

    FilterItems(FilterCallback) {
        return this.GetItems().filter(FilterCallback);
    }

    SortItems(SortCallback) {
        const SortedArray = this.GetItems().sort(SortCallback);
        localStorage.setItem(this.Storage.StorageId, JSON.stringify(SortedArray));
        return SortedArray;
    }
