    ...
    static GeneratePart(Pos = {
        ...
        if (Particle && !this.IsPlaceOccupied(X, Y) || IgnoreOccupation) {
            ...
            const NewColor = window.Effects.Powder.Enabled
                ? `rgb(${Number.Random(ColorArray[0] - window.Effects.Powder.Power, ColorArray[0] + window.Effects.Powder.Power)}, ${Number.Random(ColorArray[1] - window.Effects.Powder.Power, ColorArray[1] + window.Effects.Powder.Power)}, ${Number.Random(ColorArray[2] - window.Effects.Powder.Power, ColorArray[2] + window.Effects.Powder.Power)})`
                : Particle.Color;
...
export class Storage {
    ...
    Append() {
        ...
        if (!this.Storage.StorageId) {
            this.Storage.StorageId = (Math.random().toString(36).substring(2, 8));
        }

        const StorageArray = [];
        this.Storage.StorageItems.forEach((Item, Index) => {
            if (this.Storage.State === 1) {
                if (this.Storage.Options.IndexItems) {
                    StorageArray.push([Index, Item]);
                } else {
    ...
    Restore() {
        if (!localStorage.getItem(`Backup:${this.Storage.BackupId || this.Storage.StorageId}`)) return;
        return localStorage.getItem(`Backup:${this.Storage.BackupId || this.Storage.StorageId}`);
    }
}
