...
function PlaceParticle(MouseEvent) {
    ...
    if (MouseEvent.target === Api.ParticleContainer || (MouseEvent.target.offsetParent && MouseEvent.target.offsetParent === Api.ParticleContainer)) {
...
document.addEventListener("keydown", Event => {
    ...
    if (Key === "C") {
        ...
        Api.DisplayLabel.innerHTML = window.Displays[NewIndex];
...
ModFileInput.accept = ".js";
Api.AddNewModButton.addEventListener("click", () => {
    ModFileInput.click();
});
ModFileInput.onchange = () => {
    const File = ModFileInput.files[0];
    const Reader = new FileReader();
    Reader.onload = () => {
        Api.DevConsole.Write(`Mod loaded: ${File.name}`);
        const Mods = JSON.parse(localStorage.getItem("Mods")) || [];

        const ExistingMod = Mods.find(Item => Item.Name === File.name);
        if (ExistingMod) {
            ExistingMod.Content = Reader.result;
            ExistingMod.Enabled = true;
        } else {
            Mods.push({ Name: File.name, Content: Reader.result, Enabled: true });
        }
    
        localStorage.setItem("Mods", JSON.stringify(Mods));
        Api.DevConsole.Write(`Mod saved: ${JSON.parse(localStorage.getItem("Mods")).find(Item => Item.Name === File.name) ? "Successful" : "Failed"}`);
        eval(Reader.result);
    };
...
function Loop() {
    ...
    if (Mouse.target && Mouse.target.classList.contains("Particle")) {
        ...
        Api.PartStats.querySelector(".Color").style.backgroundColor = Api.Particle.GetElement(Mouse.target.getAttribute("Name").replace(/['"]/g, "")).Color;
    } else {
