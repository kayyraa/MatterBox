...
function Loop() {
    ...
    if (Mouse.target && Mouse.target.hasAttribute("tmp")) {
        Api.PartStats.style.opacity = "1";
        Api.PartStats.querySelector(".Name").innerHTML = Mouse.target.dataset.Name.replace(/['"]/g, "");
        Api.PartStats.querySelector(".Temp").innerHTML = `${parseFloat(Mouse.target.dataset.Temp).toFixed(1)}⁰C`;
        ...
        Api.PartStats.querySelector(".Color").style.backgroundColor = Api.Particle.GetElement(Mouse.target.dataset.Name.replace(/['"]/g, "")).Color;
    } else {
